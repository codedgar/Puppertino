...
Apple's text styles are designed to support Dynamic Type on iOS, where users can scale text to meet their needs. On the web, use relative units (rem, em) where possible and avoid hard-capping line lengths so text reflows comfortably at larger sizes.
